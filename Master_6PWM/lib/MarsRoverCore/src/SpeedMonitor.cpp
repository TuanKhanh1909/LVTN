...
void IRAM_ATTR onHallL2() {
    ...
    if (mData[1].count == 0) mData[1].first_tick = now; 
    mData[1].last_tick = now; mData[1].count++; 
    portEXIT_CRITICAL_ISR(&timerMux);
}
void IRAM_ATTR onHallL3() {
    int64_t now = esp_timer_get_time(); portENTER_CRITICAL_ISR(&timerMux);
    if (mData[2].count == 0) mData[2].first_tick = now; 
    mData[2].last_tick = now; mData[2].count++; 
    portEXIT_CRITICAL_ISR(&timerMux);
}
void IRAM_ATTR onHallR1() {
    int64_t now = esp_timer_get_time(); portENTER_CRITICAL_ISR(&timerMux);
    if (mData[3].count == 0) mData[3].first_tick = now; 
    mData[3].last_tick = now; mData[3].count++; 
    portEXIT_CRITICAL_ISR(&timerMux);
}
void IRAM_ATTR onHallR2() {
    int64_t now = esp_timer_get_time(); portENTER_CRITICAL_ISR(&timerMux);
    if (mData[4].count == 0) mData[4].first_tick = now; 
    mData[4].last_tick = now; mData[4].count++; 
    portEXIT_CRITICAL_ISR(&timerMux);
}
void IRAM_ATTR onHallR3() {
    int64_t now = esp_timer_get_time(); portENTER_CRITICAL_ISR(&timerMux);
    if (mData[5].count == 0) mData[5].first_tick = now; 
    mData[5].last_tick = now; mData[5].count++; 
    portEXIT_CRITICAL_ISR(&timerMux);
}
...
void setupSpeedMonitor() {
    ...
    pinMode(HALL_PIN_L1, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_L1), onHallL1, CHANGE);
    pinMode(HALL_PIN_L2, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_L2), onHallL2, CHANGE);
    pinMode(HALL_PIN_L3, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_L3), onHallL3, CHANGE);
    pinMode(HALL_PIN_R1, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_R1), onHallR1, CHANGE);
    pinMode(HALL_PIN_R2, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_R2), onHallR2, CHANGE);
    pinMode(HALL_PIN_R3, INPUT_PULLUP); attachInterrupt(digitalPinToInterrupt(HALL_PIN_R3), onHallR3, CHANGE);
...
int16_t calculateRPM(int motorID) {
    ...
    if (now_ms - last_calc_time >= SAMPLE_TIME_MS) {
        ...
        for(int i = 0; i < 6; i++) {
            if (localData[i].count > 1) { // Giống y chang điều kiện N > 1 bên code PID
                ...
                if (T0 > 0) {
                    ...
                    if (current_rpm < 5.0) current_rpm = 0.0; // Bỏ qua sai số khi quay quá chậm
    ...
    if(motorID >= 0 && motorID < 6) return rpm_cache[motorID];
    return 0;
}
...
int32_t getRawPulse(int motorID) {
    if(motorID >= 0 && motorID < 6) return mData[motorID].count;
    return 0;
}